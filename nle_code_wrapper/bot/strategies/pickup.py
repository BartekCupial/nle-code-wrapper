...
def pickup_wand(bot: "Bot"):
    return pickup_item(bot, G.WAND_CLASS)


def pickup_ring(bot: "Bot"):
    return pickup_item(bot, G.RING_CLASS)


def pickup_potion(bot: "Bot"):
    return pickup_item(bot, G.POTION_CLASS)


def pickup_boots(bot: "Bot"):
    ...
    return pickup_item(bot, boot_glyphs)


def pickup_horn(bot: "Bot"):
    horn_glyphs = [
        glyph
        for glyph, obj in GLYPH_TO_OBJECT.items()
    ...
    return pickup_item(bot, horn_glyphs)
