...
generation_config = GenerationConfig(
    bos_token_id=128000,
    do_sample=True,  # TODO: change back to True!
...
generated_sequence = model.generate(
    **inputs,
    ...
    use_cache=False  # TODO: using cache seems to change logits a bit (more than I would have expected)
...
raw_logits = generated_sequence.logits
...
raw_logits = torch.stack(raw_logits)  # T x B x V
logits = logits.permute(1, 0, 2)  # Rearrange to B x T x V
raw_logits = raw_logits.permute(1, 0, 2)  # Rearrange to B x T x V
...
print("True Log Probability of the Generated Sequence:", total_log_prob)

# Re-compute log probability of the generated sequence using the model's forward method
# concatenate inputs
input_ids = torch.cat([inputs["input_ids"], generated_tokens], dim=1)
attn_mask = torch.cat([inputs["attention_mask"], torch.ones_like(generated_tokens)], dim=1)

assert input_ids.shape[1] == generated_sequence.sequences.shape[1]
assert attn_mask.shape[1] == generated_sequence.sequences.shape[1]
...
model._prepare_special_tokens(generation_config, True, input_ids.device)
...
logits_man = outputs.logits[:, inputs["input_ids"].shape[1] - 1 : -1].float()  # Only the newly generated tokens

for t in range(logits_man.shape[1]):
    logits_man[:, t, :] = logits_processor(input_ids, logits_man[:, t, :])

log_softmaxed_logits = F.log_softmax(logits_man, dim=-1)  # Shape: B x T x V
gathered_log_probs = log_softmaxed_logits.gather(dim=2, index=generated_tokens.unsqueeze(-1))  # Shape: B x T x 1
gathered_log_probs = gathered_log_probs.squeeze(-1)  # Shape: B x T

masked_log_probs = gathered_log_probs.masked_fill(~padding_mask, 0.0)

total_log_prob_man = masked_log_probs.sum(dim=1)  # Shape: B

print("Manually Computed Log Probability of the Generated Sequence", total_log_prob_man)
